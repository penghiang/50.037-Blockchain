...
if __name__ == "__main__":
    ...
    newtransaction = UTXO(vk, vk2, 123, sender_key=sk)
    assert(newtransaction.validate())
    # Tests if validation works

    newtransaction2 = UTXO(vk2, vk, 3, signature=newtransaction.signature)
    assert(not newtransaction2.validate())
    # Tests if validation works for wrong signatures.

    newtransaction5 = UTXO(vk, vk2, 123)
    ...
    newtransaction4 = UTXO.from_json(x)
    assert(newtransaction4.validate())
    # Tests if from_json works.

    newtransaction3 = UTXO(vk, vk2, 123, signature=newtransaction.signature)
